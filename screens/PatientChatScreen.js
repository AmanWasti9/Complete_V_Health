...
export default function PatientChatScreen({ route, navigation }) {
      ...
      const subscription = supabase
        ...
        .on(
          "postgres_changes",
          {
            ...
            schema: "public",
            table: "chats",
            filter: `(sender_id=eq.${user.id} AND receiver_id=eq.${doctor.id}) OR (sender_id=eq.${doctor.id} AND receiver_id=eq.${user.id})`,
          },
          (payload) => {
  ...
  const formatMessage = (dbMessage) => {
    return {
      id: dbMessage.id,
      sender: dbMessage.sender_id === user.id ? "patient" : "doctor",
  ...
  const fetchMessages = async () => {
    setLoading(true)
    try {
      // Get messages between the patient and doctor
      ...
      const unreadMessages = data.filter(
        (msg) => msg.sender_id === doctor.id && msg.receiver_id === user.id && !msg.is_read,
      )
  ...
  const sendMessage = async () => {
    if (!newMessage.trim() || !user || !doctor) return
    ...
    try {
      ...
      // Simulate doctor typing after patient sends a message
      setTimeout(() => {
        setIsTyping(true)
        setTimeout(() => {
          setIsTyping(false)
  ...
  const startVideoCall = async () => {
    try {
      const callId = streamVideoService.generateCallId('patient_doctor');
      setCurrentCallId(callId);
      setOutgoingCallType('video'); // Set outgoing call type
      setShowVideoCall(true);

      // Send call notification to the doctor
  ...
  const startAudioCall = async () => {
    try {
      const callId = streamVideoService.generateCallId('patient_doctor_audio');
      setCurrentCallId(callId);
        ...
        >
          {!isPatient && !isGrouped && (
            <View style={styles.avatarContainer}>
              <Image source={{ uri: doctor.avatar_url || DEFAULT_AVATAR }} style={styles.messageAvatar} />
            </View>
          )}
          <View
            style={[
              ...
              isGrouped && (isPatient ? styles.groupedPatientBubble : styles.groupedDoctorBubble),
              !isPatient && isGrouped && styles.groupedDoctorBubbleWithSpace,
            ]}
          >
            ...
            {isLastInGroup && (
              <View style={styles.messageFooter}>
                <Text style={[styles.timestamp, isPatient ? styles.patientTimestamp : styles.doctorTimestamp]}>
        ...
        <View style={styles.header}>
          <View style={styles.headerContent}>
            ...
            <View style={styles.doctorInfo}>
              <View style={styles.avatarContainer}>
                <Image
                  source={{ uri: doctor.avatar_url || DEFAULT_AVATAR }}
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    backgroundImage: "linear-gradient(135deg, #4A90E2 0%, #357ABD 100%)",
    paddingTop: 50,
    paddingBottom: 16,
